...
import { Package, Utensils, Truck, CheckCircle2, Clock, MapPin } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ donations }) => {
  ...
  // Derive stats from donations
  const completedDonations = donations.filter(d => d.status === 'completed');
  const rescuedKg = completedDonations.reduce((acc, curr) => acc + curr.weight, 450); // 450 base
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        <h1 className="text-2xl font-semibold text-stone-900 dark:text-stone-100 tracking-tight">Good afternoon, Hotel Pritam.</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <StatCard 
          ...
          value={`${rescuedKg} kg`} 
          subtext="+45 kg this week"
          icon={Package} 
        />
        <StatCard 
          title="Meals Served" 
          value={mealsServed} 
          subtext="~4 meals per kg"
          icon={Utensils} 
        />
        <StatCard 
          title="Next Pickup" 
          value={nextPickup ? nextPickup.pickupWindow : "No pending pickups"} 
          subtext={nextPickup ? `Volunteer: ${nextPickup.assignedVolunteer?.name || 'Assigned'}` : "All clear"}
          icon={Truck} 
        />
      </div>
      ...
      <div className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-xl overflow-hidden shadow-sm">
        <div className="px-6 py-4 border-b border-stone-100 dark:border-stone-800 flex justify-between items-center">
          ...
          <button className="text-sm text-forest-600 dark:text-forest-400 hover:text-forest-700 font-medium">View All</button>
        </div>
          ...
          <table className="w-full text-sm text-left">
            ...
            <tbody className="divide-y divide-stone-100 dark:divide-stone-800">
              {recentActivity.length === 0 ? (
                  <tr>
                      <td colSpan={4} className="px-6 py-8 text-center text-stone-400">No recent activity</td>
                  </tr>
              ) : (
                    ...
                    <td className="px-6 py-4">
                      ...
                      <div className="flex gap-2 mt-1">
                        {donation.tags.map(tag => (
                          <span key={tag} className="text-xs text-stone-500 dark:text-stone-400 bg-stone-100 dark:bg-stone-800 px-1.5 py-0.5 rounded">{tag}</span>
                        ))}
                      </div>
                    </td>
      ...
      {/* Maps Grounding Feature */}
       <div className="bg-forest-50 dark:bg-forest-900/20 border border-forest-100 dark:border-forest-800/50 rounded-xl p-6">
          <div className="flex items-start gap-4">
             <div className="p-3 bg-white dark:bg-stone-800 rounded-full text-forest-600 dark:text-forest-400 shadow-sm">
                <MapPin size={24} />
             </div>
             <div className="flex-1">
                 <h3 className="font-semibold text-forest-900 dark:text-forest-200">Partner Charity Network</h3>
                 <p className="text-forest-700 dark:text-forest-400 text-sm mt-1 mb-4">
                    Verify nearby distribution centers and food banks in our network using real-time maps data.
                 </p>
                 ...
                 <button 
                    onClick={handleLocateCharities}
                    disabled={groundingLoading}
                    className="text-sm font-medium bg-forest-600 text-white px-4 py-2 rounded-lg hover:bg-forest-700 transition-colors disabled:opacity-50">
