...
import { Video, Loader2, Share2, Star, MessageSquare } from 'lucide-react';
...
const History: React.FC<HistoryProps> = ({ donations, onRate }) => {
  ...
  const submitRating = () => {
      if (ratingModalId && tempRating > 0) {
          onRate(ratingModalId, tempRating, tempReview);
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        {historyItems.map((item) => (
            <div key={item.id} className="bg-white dark:bg-stone-900 border border-stone-200 dark:border-stone-800 rounded-xl overflow-hidden shadow-sm flex flex-col md:flex-row">
                <div className="md:w-48 h-48 md:h-auto relative bg-stone-100 dark:bg-stone-800">
                   {videos[item.id] ? (
                       <video src={videos[item.id]} controls className="w-full h-full object-cover" />
                   ) : (
                       <img src={item.imageUrl || "https://placehold.co/300x200?text=Food"} alt="Food" className="w-full h-full object-cover" />
                   )}
                   ...
                   {!videos[item.id] && item.imageUrl && (
                       <div className="absolute inset-0 bg-black/0 hover:bg-black/20 transition-colors flex items-center justify-center group">
                            <button 
                                onClick={() => handleGenerateVideo(item.id, item.imageUrl!)}
                                disabled={!!generatingId}
                                className="opacity-0 group-hover:opacity-100 bg-white dark:bg-stone-800 text-forest-800 dark:text-forest-400 px-4 py-2 rounded-full font-medium shadow-lg transform translate-y-2 group-hover:translate-y-0 transition-all flex items-center gap-2 disabled:opacity-50"
                            >
                ...
                <div className="p-6 flex-1 flex flex-col justify-between">
                    <div>
                        ...
                        <div className="mt-4 flex gap-2">
                             ...
                             <span className="inline-block px-2 py-1 bg-stone-100 dark:bg-stone-800 text-stone-600 dark:text-stone-400 text-xs rounded border border-stone-200 dark:border-stone-700">Verified by Partner</span>
                        </div>
                    </div>
                    ...
                    <div className="mt-4 pt-4 border-t border-stone-100 dark:border-stone-800 flex flex-wrap gap-4 items-center justify-between">
                         {videos[item.id] && (
                             <button className="text-sm flex items-center gap-2 text-forest-700 dark:text-forest-400 font-medium hover:text-forest-800 dark:hover:text-forest-300">
                         ...
                         <div className="ml-auto">
                            {item.rating ? (
                                <div className="flex items-center gap-2">
                                    <div className="flex">
                                        {[1,2,3,4,5].map(star => (
                                            <Star key={star} size={16} className={star <= item.rating! ? "text-amber-400 fill-amber-400" : "text-stone-300 dark:text-stone-600"} />
                                        ))}
                                    </div>
                                ...
                                <button 
                                    ...
                                    className="text-sm bg-stone-100 dark:bg-stone-800 hover:bg-stone-200 dark:hover:bg-stone-700 text-stone-700 dark:text-stone-300 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-2 font-medium"
                                >
                                    <Star size={14} /> Rate Volunteer
                                </button>
                            )}
                         </div>
                    </div>
      ...
      {ratingModalId && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
              <div className="bg-white dark:bg-stone-900 rounded-xl shadow-xl w-full max-w-sm overflow-hidden animate-in zoom-in-95 duration-200 p-6">
                  ...
                  <div className="flex justify-center gap-2 mb-6">
                      {[1,2,3,4,5].map(star => (
                          ...
                          >
                              <Star 
                                size={32} 
                                className={`${star <= tempRating ? 'text-amber-400 fill-amber-400' : 'text-stone-200 dark:text-stone-700'}`} 
                                strokeWidth={1.5}
                              />
                          </button>
                      ))}
                  ...
                  <div className="mb-6 relative">
                      ...
                      <textarea 
                        ...
                        className="w-full bg-stone-50 dark:bg-stone-950 border border-stone-200 dark:border-stone-800 rounded-lg p-3 pl-9 text-sm focus:outline-none focus:border-forest-500 dark:text-stone-200 resize-none h-24"
                      />
                  </div>
                  ...
                  <div className="flex gap-3">
                      <button 
                        ...
                        className="flex-1 py-2.5 bg-stone-100 dark:bg-stone-800 text-stone-600 dark:text-stone-300 font-medium rounded-lg hover:bg-stone-200 dark:hover:bg-stone-700 transition-colors"
                      >
                          Cancel
                      </button>
                      <button 
                        ...
                        className="flex-1 py-2.5 bg-forest-600 text-white font-medium rounded-lg hover:bg-forest-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                      >
                          Submit
                      </button>
                  </div>
